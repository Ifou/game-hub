...
import { User, Discussion, Game, Comment } from '@/types';
...
export default function UserForumShow({ profileUser, discussion, auth }: Props) {
    const [showCommentForm, setShowCommentForm] = useState(false);

    const { data, setData, post, processing, errors, reset } = useForm({
        content: '',
        commentable_type: 'App\\Models\\Discussion',
        commentable_id: discussion.id.toString(),
    });
    ...
    const isOwnDiscussion = auth.user?.id === discussion.user_id;

    return (
        <AppLayout>
            <Head title={`${discussion.title} - ${profileUser.name}'s Forum`} />
                            ...
                            <div className="flex items-start justify-between mb-4">
                                <div className="flex-1">
                                    <div className="flex items-center gap-2 mb-3">
                                        ...
                                        {discussion.game && (
                                            <Link
                                                href={route('games.show', discussion.game.id)}
