...
export default function UserForumEdit({ profileUser, discussion, userGames, auth }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        title: discussion.title,
        content: discussion.content,
        category: discussion.category,
        game_id: discussion.game?.id?.toString() || '',
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('users.forum.update', [profileUser.id, discussion.id]));
    };

    return (
        <AppLayout>
            <Head title={`Edit Discussion - ${profileUser.name}'s Forum`} />
                ...
                <div className="max-w-4xl mx-auto sm:px-6 lg:px-8">
                    ...
                    <div className="mb-6">
                        <nav className="flex space-x-2 text-sm text-gray-500">
                            ...
                            <span>/</span>
                            <Link href={route('users.forum.show', [profileUser.id, discussion.id])} className="hover:text-gray-700">
                                {discussion.title}
                            </Link>
                            <span>/</span>
